...
import uon.vehiclehiresystem.models.Lorry;
import uon.vehiclehiresystem.models.MiniBus;
import uon.vehiclehiresystem.models.Vehicle;
import uon.vehiclehiresystem.utility.Validator;
import uon.vehiclehiresystem.views.FrameAddVehicle;
...
//        int topSpeed, int dailyHireRate, String make, String model, boolean isHired
        ...
        Messages.showMessage("Added Successfully", frameAddVehicle);
    ...
    private boolean validateForm(){
        ...
        if(!Validator.validateUniqueRegNo(frameAddVehicle.getTxtRegNumber())){
            Messages.showErrorMessage(frameAddVehicle.getLblRegNumber().getText() +
                    " must be unique!", frameAddVehicle);
            isUnfinished = true;
        }
        if(!Validator.validateIsNumber(frameAddVehicle.getTxtDailyHireRate())){
            Messages.showErrorMessage( frameAddVehicle.getLblDailyHireRate().getText() +
                    " must be a number!", frameAddVehicle);
            isUnfinished = true;
        }
        if(!Validator.validateNotNull(frameAddVehicle.getTxtMake())){
            Messages.showErrorMessage( frameAddVehicle.getLblMake().getText() +
                    " is missing!", frameAddVehicle);
            isUnfinished = true;
        }
        if(!Validator.validateNotNull(frameAddVehicle.getTxtModel())){
            Messages.showErrorMessage( frameAddVehicle.getLblModel().getText() +
                    " is missing!", frameAddVehicle);
            isUnfinished = true;
        }

        if(!Validator.validateIsNumber(frameAddVehicle.getTxtTopSpeed())){
            Messages.showErrorMessage( frameAddVehicle.getLblTopSpeed().getText() +
                    " must be a number!", frameAddVehicle);
            isUnfinished = true;
        }
        if(!Validator.validateNotNull(frameAddVehicle.getTxtSpecial1())){
            Messages.showErrorMessage( frameAddVehicle.getLblSpecial1().getText() +
                    " is missing!", frameAddVehicle);
            isUnfinished = true;
        }
        ...
        if(type == Vehicle.TYPE_CAR && !Validator.validateNotNull(frameAddVehicle.getTxtSpecial2())){
            Messages.showErrorMessage( frameAddVehicle.getLblSpecial2().getText() +
                    " is missing!", frameAddVehicle);
            isUnfinished = true;
        }
